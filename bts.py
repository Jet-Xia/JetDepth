...
class encoder(nn.Module):
    def __init__(self, params):
        ...
        elif params.encoder == 'mobilenetv3_bts':
            self.base_model = models.mobilenet_v3_large(pretrained=True).features
            self.feat_inds = [2, 4, 7, 11, 17]
            self.feat_out_channels = [16, 24, 40, 80, 960]
            self.feat_names = []
        else:
    ...
    def forward(self, x):
        ...
        for k, v in self.base_model._modules.items():
            ...
            elif self.params.encoder == 'mobilenetv3_bts':
                if i == 2 or i == 4 or i == 7 or i == 11 or i == 17:
                    skip_feat.append(feature)
            else:
